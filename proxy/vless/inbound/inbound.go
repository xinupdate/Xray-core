...
func (h *Handler) Process(ctx context.Context, network net.Network, connection internet.Connection, dispatcher routing.Dispatcher) error {
	...
	header, err := vlessConn.GetHeader()
	if err != nil {
		return err
	}

	// Handle SMUX command
	if header.Command == protocol.RequestCommandSmux {
	...
	//	if request, err = vlessConn.GetHeader(); err != nil {
	//		return err
	//	}
	//}
	...
	postRequest := func() error {
		...
		err = buf.Copy(buf.NewReader(vlessConn), serverWriter, buf.UpdateActivity(timer))
		if err != nil {
	...
	getResponse := func() error {
		...
		if err := buf.Copy(serverReader, buf.NewWriter(vlessConn), buf.UpdateActivity(timer)); err != nil {
