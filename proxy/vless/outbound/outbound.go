...
func (h *Handler) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	var vlessConn internet.Connection
	var useSmux bool
	...
	if err := retry.ExponentialBackoff(5, 200).On(func() error {
		...
		useSmux = rec.UseSmux()
		if conn, err := h.connManager.GetConnection(ctx, rec.Destination(), dialer); err != nil {
			return err
		} else {
			...
			vlessConn = NewOutboundConn(conn, request)
		}
		return nil
	}); err != nil {
	...
	if useSmux {
		smuxSession, err := smux.Client(vlessConn, nil)
		if err != nil {
	...
	postRequest := func() error {
		...
		if err := buf.Copy(clientReader, buf.NewWriter(vlessConn), buf.UpdateActivity(timer)); err != nil {
	...
	getResponse := func() error {
		...
		//}
		err := buf.Copy(buf.NewReader(vlessConn), clientWriter, buf.UpdateActivity(timer))
		if err != nil {
